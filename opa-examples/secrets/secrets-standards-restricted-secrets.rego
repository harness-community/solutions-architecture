# Policy: Secrets Standards - Restricted Secrets Management
package secret
...
#   restricted_secret_ids = String value of the secret to which restrict access
#   - NOTE: (Account Secret start with `account.` and Org Secret start with `org.`)

approved_users = []
approved_groups = []
approved_secret_tags = {}
restricted_secret_ids = []
...
deny[msg] {
	...
	secret := input.secret
	array_contains(restricted_secret_ids, return_notated_obj(secret))
...
return_count_if_elem_in_list(items, eval_arr) := 0 if {
  any([eval_arr == {}, count(eval_arr) == 0, eval_arr == ""])
} else := output if {
	is_array(items)
