# Policy: Recursively Restrict Step Connectors except in valid Template
package pipeline
...
filter_object 		= "spec.connector"
forbidden_items 	= ["account.default_github_access", "account.DockerHub", "account.harnessImageExternal", "account.harnessImage", "account.azure"]
error_message(step, step_object) := sprintf("Failed: The step '%s' in stage '%s' has a Harness Connector Definition '%s' that is forbidden and only valid when used with an approved template %s. ", [step.name, step.stage.name, step_object, approved_templates])
