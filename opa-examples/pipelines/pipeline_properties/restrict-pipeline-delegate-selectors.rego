# Policy: Restrict Pipeline Delegate Selectors
package pipeline
...
filter_object 	    = "delegateSelectors"
forbidden_items     = ["bootstrap"]
error_message(pipeline, eval_obj) :=sprintf("Failed: The pipeline '%s' has a Delegate Selector '%s' that is forbidden and only valid when used with an approved template %s. ", [pipeline.name, eval_obj, approved_templates])
